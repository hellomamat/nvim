...
if vim.g.vscode then
	...
	map("n", "<leader>qq", function() vscode.action("workbench.action.closeAllEditors") end, opts)

	-- Find
	map("n", "<leader>ff", function() vscode.action("workbench.action.quickOpen") end, opts)
	map("n", "<leader>fg", function() vscode.action("workbench.action.findInFiles") end, opts)
	map("n", "<leader>fb", function() vscode.action("workbench.action.showAllEditors") end, opts)

	-- Explorer
	map("n", "<leader>e", function() vscode.action("workbench.action.toggleSidebarVisibility") end, opts)

	-- Tab navigation
	map("n", "<leader>n", function() vscode.action("workbench.action.nextEditor") end, opts)
	map("n", "<leader>p", function() vscode.action("workbench.action.previousEditor") end, opts)
	map("n", "<leader>3", function() vscode.action("workbench.action.closeActiveEditor") end, opts)
	map("n", "<Space>0", function() vscode.action("workbench.action.nextEditor") end, opts)
	map("n", "<Space>9", function() vscode.action("workbench.action.previousEditor") end, opts)

	-- Split
	map("n", "<leader>tt", function() vscode.action("workbench.action.terminal.toggleTerminal") end, opts)
	...
	map("n", "K", function() vscode.action("editor.action.showHover") end, opts)
	map("n", "<leader>rn", function() vscode.action("editor.action.rename") end, opts)
	map("n", "<leader>ca", function() vscode.action("editor.action.quickFix") end, opts)
	map("n", "<Space>f", function() vscode.action("editor.action.formatDocument") end, opts)
	map("v", "<Space>f", function() vscode.action("editor.action.formatSelection") end, opts)
	map("n", "<Space>E", function() vscode.action("editor.action.showHover") end, opts)
...
-- ========================= --
			...
			config = function()
				...
				require("nvim-tree").setup({
					...
					on_attach = function(bufnr)
						...
						vim.keymap.set("n", "o", api.node.open.edit, nopts("Open without changing root"))
						vim.keymap.set("n", "O", api.node.open.vertical, nopts("Open vertical without changing root"))
						vim.keymap.set("n", "C", api.tree.change_root_to_node, nopts("Change root to current node"))
						vim.keymap.set("n", "|", api.node.open.vertical, nopts("Open in vertical split"))
						vim.keymap.set("n", "-", api.node.open.horizontal, nopts("Open in horizontal split"))
					end,
				})
			end,
		},
	...
	map("n", "<leader>lh", function()
		...
		if not stats or not stats.plugins then return end
		local sorted = {}
